    ...
    <script>
        $(document).ready(function() {
                            ...
                            let card = `
                                <div class="bg-white rounded-lg shadow-md border border-gray-200 p-4 hover:shadow-lg transition-shadow">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <button class="flex-1 bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded text-sm delete-btn" data-id="${order.id}">
                                            Delete
                                        </button>
                                    </div>
                                </div>
            ...
            $('#confirmDeleteBtn').click(function() {
                if (isDeleting || !selectedOrderId) return;

                isDeleting = true;

                $.ajax({
                    url: `/orders/${selectedOrderId}/delete`,
                    method: 'POST',
                    dataType: 'json',
                    success: function(data) {
                        if (data.success) {
